...
* There is also a new :meth:`str.format_map` method that extends the
  ...
  :class:`~collections.defaultdict`, :class:`~configparser.ConfigParser`,
  or :mod:`!dbm`.  It is also useful with custom :class:`dict` subclasses that
  normalize keys before look-up or that supply a :meth:`__missing__` method for
  unknown keys::
