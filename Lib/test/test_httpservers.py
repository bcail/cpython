...
class SimpleHTTPServerTestCase(BaseTestCase):
    ...
    def test_undecodable_parameter(self):
        ...
        self.assertRegex(response, fr'listing for {self.base_url}/\?x=123'.encode('latin1'))
        # now the bogus encoding
        response = self.request(self.base_url + '/?x=%bb').read()
        self.assertRegex(response, fr'listing for {self.base_url}/\?x=\xef\xbf\xbd'.encode('latin1'))
